...
import { MAX_ALBUM_PHOTOS } from "../schemas";
...
// ponytail: near-duplicate of PostImagePicker (upload plumbing is identical,
// only the field name/kind/limit differ) — not worth a shared abstraction
// for two call sites; revisit if a third one shows up.
export function AlbumImagePicker({
  disabled,
  onPendingChange,
  maxItems = MAX_ALBUM_PHOTOS,
}: {
  disabled?: boolean;
  ...
  async function handleChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setError(null);
    const slotsLeft = maxItems - items.length;
    const accepted = files.slice(0, slotsLeft);
    if (files.length > slotsLeft)
      setError(`Puoi aggiungere al massimo ${maxItems} foto.`);
    for (const file of accepted) {
      ...
      try {
        ...
        await upload(`album-${id}.${file.name.split(".").pop()}`, file, {
          access: "public",
  ...
  return (
    <div className="stack">
      <input type="hidden" name="assetIds" value={JSON.stringify(readyIds)} />
      {items.length > 0 && (
