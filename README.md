...
9. **Completata:** blog (`/blog`), post lunghi separati dal feed a status breve, con visibilità pubblico/amici/privato per singolo post (indipendente dagli altri post dello stesso autore).
...
**Notifiche push (Web Push)**: implementate a livello di codice (`src/features/push/`, service worker in `public/sw.js`), ma **richiedono una coppia di chiavi VAPID non ancora impostata in produzione**. Per generarle: `npx web-push generate-vapid-keys`, poi impostare `VAPID_PUBLIC_KEY` e `VAPID_PRIVATE_KEY` (e opzionalmente `VAPID_SUBJECT`, un `mailto:` di contatto) tra le variabili d'ambiente Vercel. Finché non sono impostate, la sezione "Notifiche push" in `/settings` resta nascosta e le notifiche continuano a funzionare normalmente in-app (nessun impatto su nient'altro): è puro potenziamento opt-in, non un blocco.
